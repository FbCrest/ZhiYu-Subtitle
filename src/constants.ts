...
Ensure titles are concise (5-7 words max) and summaries are brief (1-2 sentences). Focus on major segmentation points. Return ONLY the formatted chapter lines following this exact single-line structure. Do not include any introductory text, concluding remarks, blank lines, lists, or any other text or formatting.`,
  ...
  },
  {
    ...
    content: `Transcribe all spoken content in this {contentType} and translate each segment directly into TARGET_LANGUAGE. For each segment:
...
Format the output as a sequential transcript. Each line MUST strictly follow the format: [MMmSSsNNNms - MMmSSsNNNms] Translated text in TARGET_LANGUAGE. For example: [00m30s000ms - 00m35s500ms] This is the translated speech. Always use leading zeros for minutes and seconds (e.g., 00m05s100ms, not 0m5s100ms). **CRITICAL: Break the translation into VERY SHORT segments. Focus on natural pauses and short phrases. Aim for segments that are typically only a few words long.** Return ONLY the formatted transcript lines with translations. Do not include any headers, summaries, introductions, or any other text whatsoever.
IMPORTANT: If there is no speech or spoken content in the audio, return an empty array []. Do not return timestamps with empty text or placeholder text.`,
    isSystem: true
  }
];
...
export const AI_MODELS = [
  ...
  },
  {
    ...
    tag: 'STABLE',
    color: 'indigo',
    description: 'Cân bằng, ổn định và đáng tin cậy',
    speed: 'Bình Thường',
    accuracy: 'Tốt'
  },
  {
    ...
    tag: 'FAST',
    color: 'purple',
    description: 'Nhanh nhất, phù hợp xử lý nhanh',
    speed: 'Rất Nhanh',
    accuracy: 'Khá'
  },
];
